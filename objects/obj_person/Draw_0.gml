...
if(array_length(associatedNodes) != 0){
	for(var i=0; i<array_length(associatedNodes); i++){
		var node = associatedNodes[i];
		//print(node.x)
		draw_line_width_color (x, y, node.x, node.y, 4, c_fuchsia, c_blue);			
	}
}
